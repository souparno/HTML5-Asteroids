Stage(function(canvas, context) {
  ...
  Sprite = function() {
    ...
    this.findCollisionCanidates = function() {
      ...
      var canidates = [];
      if (cn.nextSprite)
        canidates.push(cn.nextSprite);
      if (cn.north.nextSprite)
        canidates.push(cn.north.nextSprite);
      if (cn.south.nextSprite)
        canidates.push(cn.south.nextSprite);
      if (cn.east.nextSprite)
        canidates.push(cn.east.nextSprite);
      if (cn.west.nextSprite)
        canidates.push(cn.west.nextSprite);
      if (cn.north.east.nextSprite)
        canidates.push(cn.north.east.nextSprite);
      if (cn.north.west.nextSprite)
        canidates.push(cn.north.west.nextSprite);
      if (cn.south.east.nextSprite)
        canidates.push(cn.south.east.nextSprite);
      if (cn.south.west.nextSprite)
        canidates.push(cn.south.west.nextSprite);
      return canidates
    };
  ...
  BigAlien = function() {
    ...
    this.children.top.init("bigalien_top", [-8, -4, -6, -6,
      6, -6,
      8, -4
    ]);
  ...
  Asteroid = function() {
    this.init("asteroid", [-10, 0, -5, 7, -3, 4,
      1, 10,
      5, 4,
      10, 0,
      5, -6,
  ...
  Game = {
    ...
    FSM: {
      ...
      waiting: function() {
        Text.renderText('Touch Screen to Start', 36, Game.canvasWidth / 2 - 270, Game.canvasHeight / 2);
      ...
      new_level: function() {
        ...
        if (Date.now() - this.timer > 1000) {
          this.timer = null;
          Game.totalAsteroids++;
          if (Game.totalAsteroids > 12)
            Game.totalAsteroids = 12;
          Game.spawnAsteroids();
          this.state = 'run';
        }
      },
